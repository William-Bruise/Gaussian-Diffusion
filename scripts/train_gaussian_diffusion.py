...
def main():
    ...
    encoder_type = ckpt.get('encoder_type', 'global')
    enc = build_gaussian_encoder(encoder_type, num_gaussians).to(device)
    enc.load_state_dict(ckpt['encoder'])
    enc.eval()
    for p_ in enc.parameters(): p_.requires_grad = False

    gparam = GaussianParametrization(GaussianParamConfig(num_gaussians=num_gaussians))
    ...
    for ep in range(args.epochs):
        for it, (x, _) in enumerate(dl):
            ...
            if it % 50 == 0:
                ...
                with torch.no_grad():
                    zs = ddpm.sample((1, num_gaussians, 8), device)
    ...
    torch.save({'ddpm': ddpm.state_dict(), 'num_gaussians': num_gaussians}, 'outputs/checkpoints/diffusion.pt')
